...
export default function AdvancedVirtualTryOn() {
  ...
  const products = {
    shirts: [
      { id: 's1', name: 'T-Shirt', color: '#3b82f6', secondaryColor: '#2563eb', image: '/api/placeholder/100/100' },
      { id: 's2', name: 'Polo', color: '#ef4444', secondaryColor: '#dc2626', image: '/api/placeholder/100/100' },
      { id: 's3', name: 'Dress Shirt', color: '#10b981', secondaryColor: '#059669', image: '/api/placeholder/100/100' }
    ],
    pants: [
      { id: 'p1', name: 'Jeans', color: '#1f2937', secondaryColor: '#111827', image: '/api/placeholder/100/100' },
      { id: 'p2', name: 'Chinos', color: '#1e40af', secondaryColor: '#1e3a8a', image: '/api/placeholder/100/100' }
    ],
    shoes: [
      { id: 'sh1', name: 'Sneakers', color: '#f9fafb', secondaryColor: '#f3f4f6', image: '/api/placeholder/100/100' },
      { id: 'sh2', name: 'Boots', color: '#111827', secondaryColor: '#030712', image: '/api/placeholder/100/100' }
    ]
  };
    ...
    <div className="flex flex-col md:flex-row w-full gap-6 p-4">
      ...
      <div className="w-full md:w-1/2 flex flex-col items-center">
        ...
        <div className="flex gap-4 mb-4">
          <button 
            className={`px-4 py-2 rounded-lg ${gender === 'male' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            onClick={() => setGender('male')}
          >
            Male
          </button>
          <button 
            className={`px-4 py-2 rounded-lg ${gender === 'female' ? 'bg-pink-500 text-white' : 'bg-gray-200'}`}
          ...
          >
            ...
            <g transform="translate(0,2)">
              ...
              {wornItems.shoes && (
                <>
                  <path 
                    d={`M${gender === 'female' ? '80' : '78'},285 L${gender === 'female' ? '86' : '84'},285`} 
                    stroke={wornItems.shoes.secondaryColor} 
                    strokeWidth="1.5" 
                  />
                  <path 
                    d={`M${gender === 'female' ? '120' : '122'},285 L${gender === 'female' ? '114' : '116'},285`} 
                    stroke={wornItems.shoes.secondaryColor} 
                    strokeWidth="1.5" 
                  />
                </>
            ...
            <g transform={`translate(0,${breatheOffset})`}>
              ...
              {wornItems.shirt && (
                <>
                  <path 
                    d={gender === 'female' ? "M95,105 L105,105" : "M95,105 L105,105"} 
                    stroke={wornItems.shirt.secondaryColor} 
                    strokeWidth="1" 
                  />
                  <path 
                    d={gender === 'female' ? "M92,120 L108,120" : "M90,120 L110,120"} 
                    stroke={wornItems.shirt.secondaryColor} 
                    strokeWidth="0.8" 
                  />
                </>
        ...
        <div className="mt-4 p-4 bg-white rounded-lg shadow">
          ...
          <ul className="text-sm">
            <li>
              <span className="font-medium">Shirt:</span> {wornItems.shirt ? wornItems.shirt.name : 'None'}
              {wornItems.shirt?.isCustom && " (Custom Color)"}
            </li>
            <li>
              <span className="font-medium">Pants:</span> {wornItems.pants ? wornItems.pants.name : 'None'}
              {wornItems.pants?.isCustom && " (Custom Color)"}
            </li>
            <li>
              <span className="font-medium">Shoes:</span> {wornItems.shoes ? wornItems.shoes.name : 'None'}
        ...
        <div className="mb-6 p-4 bg-white rounded-lg shadow">
          ...
          <div className="flex flex-wrap gap-4 mt-2">
            {products.shirts.map(shirt => (
              <div 
                key={shirt.id}
                className={`w-24 h-24 rounded-lg cursor-pointer hover:scale-105 transition-transform shadow-md ${wornItems.shirt?.id === shirt.id && !wornItems.shirt?.isCustom ? 'ring-2 ring-blue-500' : ''}`}
        ...
        <div className="mb-6 p-4 bg-white rounded-lg shadow">
          ...
          <div className="flex flex-wrap gap-4 mt-2">
            {products.pants.map(pant => (
              <div 
                key={pant.id}
                className={`w-24 h-24 rounded-lg cursor-pointer hover:scale-105 transition-transform shadow-md ${wornItems.pants?.id === pant.id && !wornItems.pants?.isCustom ? 'ring-2 ring-blue-500' : ''}`}
        ...
        <div className="mb-6 p-4 bg-white rounded-lg shadow">
          ...
          <div className="flex flex-wrap gap-4 mt-2">
            {products.shoes.map(shoe => (
              <div 
                key={shoe.id}
                className={`w-24 h-24 rounded-lg cursor-pointer hover:scale-105 transition-transform shadow-md ${wornItems.shoes?.id === shoe.id && !wornItems.shoes?.isCustom ? 'ring-2 ring-blue-500' : ''}`}
