...
import { ShoppingCart, Search, Heart, Sparkles, Sun, Moon, Package } from 'lucide-react';
import { useTheme } from '../../context/ThemeContext';
import { useDispatch, useSelector } from 'react-redux';
import { toggleCart } from '../../store/cartSlice';
import { toggleWishlist, fetchWishlist, removeFromWishlist } from '../../store/wishlistSlice';
import { fetchProducts, fetchSearchResults, clearSearchResults } from '../../store/productsSlice';
import { NavLink, useNavigate } from 'react-router-dom';
import { CartSidebar } from '../Cart/CartSidebar'; // Adjust path
...
function Header() {
  ...
  const { wishlistItems, isOpen: isWishlistOpen } = useSelector((state) => state.wishlist);
  ...
  return (
    <>
      ...
      >
        <Stack
          sx={{ width: { xs: '100%', sm: '24rem' }, height: '100%', bgcolor: 'background.paper' }}
        >
          <Stack direction="row" justifyContent="space-between" alignItems="center" p={2} borderBottom={1} borderColor="divider">
            <Typography variant="h6" fontWeight="bold">Wishlist</Typography>
            <IconButton onClick={() => dispatch(toggleWishlist())} color="inherit">
              <X size={24} />
            </IconButton>
          </Stack>
          ...
          <Stack flex={1} overflow="auto" p={2}>
            {wishlistItems.length === 0 ? (
              <Typography textAlign="center" color="text.secondary">Your wishlist is empty</Typography>
            ) : (
              <Stack spacing={2}>
                {wishlistItems.map((item) => (
                  <Stack key={item._id} direction="row" alignItems="center" spacing={2}>
                    ...
                    <IconButton onClick={() => dispatch(removeFromWishlist(item._id))} color="inherit">
                      <X size={20} />
                    </IconButton>
                  </Stack>
                ))}
